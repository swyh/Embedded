...
#define	 MSG_QUEUE_SIZE					20		 /* Message Queue size								   */		
...
OS_EVENT     *RandomSem;
OS_EVENT	 *myMessageQueue[8];	// 8개의 massage queue를 담기위한 event conrol block
void		 *MsgQueueTbl[8][20];	// 각 message queue에 대한 table 공간
...
static  void  TaskStartCreateTasks(void)
{
	...
	// 8개의 message queue를 생성
	for (i = 0; i < 8; i++)
		myMessageQueue[i] = OSQCreate(&MsgQueueTbl[i][0], MSG_QUEUE_SIZE);
...
void  DecisionTask(void *pdata)	// 4개의 RandomTask로부터 random한 숫자를 받아, 가장 작은 숫자를 준 task에게 'W' 전달. 나머지 task에는 'L' 전달
{
	...
	for (;;) {
		for (i = 0; i < 4; i++) {
			value = *(char*)OSQPend(myMessageQueue[i], 0, &err);	//mailbox 0 ~ 4번 순서대로 기다림
		...
		for (i = 0; i < 4; i++) {
			...
			OSQPost(myMessageQueue[i + 4], &task_value[i]);	// 각 task에게 문자 전달 위해 mailbox로 보냄
		}
...
void RandomTask(void *pdata) {	// random한 값을 생성하여 출력하고, DecisionTask에게 보냄. 
	...
	for (;;) {
		...
		OSQPost(myMessageQueue[*(char*)pdata], &rnd);	// random 값을 각 mailbox로 보냄


		alphabet = *(char*)OSQPend(myMessageQueue[*(char*)pdata + 4], 0, &err);	// mailbox로부터 값을 기다림
